...
class OCEAN():
    ...
    def __init__(self, openness=None, conscientiousness=None, extraversion=None, agreeableness=None, neuroticism=None, personality=np.zeros(5)):
        # print 'OCEAN openness: ' + str(openness)
        # print 'OCEAN conscientiousness: ' + str(conscientiousness)
        # print 'OCEAN extraversion: ' + str(extraversion)
        # print 'OCEAN agreeableness: ' + str(agreeableness)
        # print 'OCEAN neuroticism: ' + str(neuroticism)
        ...
        # print 'Neuroticism: ' + str(self.neuroticism)
...
class OCC():
    def __init__(self,
                 ...
                 pad=None):
        if all([admiration, gloating, gratification, gratitude, hope, happy_for, joy, liking, love,  pride, relief, satisfaction,
    ...
    def pad(self):
        if self.pad:
            return PAD(pleasure=self.pad[0], arousal=self.pad[1], dominance=self.pad[3])
        else:
            self.emotion = {'P': 0.0, 'A': 0.0, 'D': 0.0}
            for attr in self.__dict__:
                temp = self.__dict__[attr]
                self.emotion = {k: self.emotion[k]+temp*self.pad_map[attr][k] for k in temp}
            self.emotion = {k: self.emotion[k]/24 for k in self.emotion}
            return PAD(pleasure=self.emotion[0], arousal=self.emotion[1], dominance=self.emotion[2])